...
| Brand logo uploads | `mcp__claude_ai_pika__upload_asset` rejects SVG (`image/svg+xml`). Use PNG/JPG/WebP logo exports from `build-a-brand`, or rasterize SVGs to PNG with `mcp__claude_ai_pika__html_to_png` before uploading. |
...
| Lower-third overlay | Prefer `mcp__claude_ai_pika__edit_video_compose` for transparent alpha overlays; local ffmpeg is only an emergency fallback if MCP compose is unavailable. |
