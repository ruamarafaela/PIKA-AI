...
Run this probe before Step 7 and before any paid `mcp__claude_ai_pika__generate_reference_video` call. It applies to caller-supplied `avatar_url`, a future `identity_avatar_url` default, or any creator portrait chosen as `@Image1`. The built-in fallback is already a non-IP stylized creator, but still document its source in the final summary.
...
Right before the generate call, fetch the user's voice metadata: call `identity_voice_info`. This returns JSON `{ voice_id, platform, sample_url }`; fields may be null when no routable voice/sample is on file.

- If `sample_url` is non-null → capture it as `voice_sample_url` and pass it on the next call's `reference_audio` array. Both seedance and kling accept `reference_audio` (seedance up to 3, ≤15s combined; kling up to 8). The model uses the sample to clone the speaker's timbre for the lip-sync.
- If `sample_url` is null → skip; the model uses its default voice.

Always get `identity_voice_info` fresh right before step 7 — do NOT cache or reuse a stale sample URL across runs.
...
- `reference_images: [avatar_url]` (the new plural form; `reference_image: <url>` is still accepted as a deprecated single-image alias for back-compat)
